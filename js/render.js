...
function renderFin(p){
  ...
  <div class="card">
    <h3>Orçado × Gasto por etapa</h3>
    <p class="card-note">As etapas vêm do cronograma (fixas). O orçamento é definido por etapa — categorias só agrupam os gastos dentro dela. Etapas sem orçamento definido entram no total gasto, mas não no saldo.</p>
    ...
    ${etapas.length?etapas.map(etapa=>{
      ...
      const semOrcamento=ep===0, estouro=!semOrcamento && erOrc>ep;
      ...
      const localMax=Math.max(ep,er,1);
      return `<div class="fin-etapa">
        <div class="fin-etapa-head">
          <h4>${etapa}</h4>
          <div style="display:flex;align-items:center;gap:12px">
            <span class="tot"><b>${fmt(er)}</b> de ${fmt(ep)} orçado</span>
            ${finPctBadge(semOrcamento?null:Math.round(erOrc/ep*100), estouro)}
            ${podeEditFin?`<button class="mini-btn" onclick="openEditOrcamentoEtapa(${p.id},'${etapa}')">Editar orçamento</button>`:''}
          </div>
        </div>
        <div class="fin-bars" style="margin-bottom:14px">
          <div class="fin-bar bar-prev"><i style="width:${ep/localMax*100}%"></i></div>
          <div class="fin-bar bar-real"><i style="width:${er/localMax*100}%"></i></div>
        </div>
        ${cs.length?cs.map((c,i)=>{
          const real=realizado(c);
          return `<div class="fin-row" style="grid-template-columns:1fr 120px ${(podeEditFin||podeExclFin)?'160px':'0'}">
            <div>${c.nome}<br>
            ...
            <div class="num" style="font-size:12px"><b>${fmt(real)}</b></div>
            ${(podeEditFin||podeExclFin)?`<div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end">
              ${podeEditFin?`<button class="mini-btn" onclick="openGasto(${p.id},'${etapa}',${i})">+ Lançar gasto</button>`:''}
  ...
  <div class="card">
    ...
    <p class="card-note">Soma de cada categoria em todas as etapas do projeto — só o gasto, já que o orçamento agora é por etapa.</p>
    ${totais.length?totais.map(t=>`<div class="fin-row" style="grid-template-columns:1fr 120px">
        <div>${t.nome}</div>
        <div class="num" style="font-size:12px"><b>${fmt(t.real)}</b></div>
      </div>`).join(''):`<div class="mut" style="font-size:12px">Nenhum lançamento ainda.</div>`}
  </div>`;
}
